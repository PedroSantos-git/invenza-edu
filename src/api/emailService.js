...
export const EmailService = {
  async send({ to, cc, subject, body }) {
    try {
      ...
      // Usamos uma abordagem que não falha com 406/PGRST116 se a linha não existir
      ...
      if (!config || !config.host || !config.user || !config.pass) {
        throw new Error('Configuração de email (SMTP) não encontrada ou incompleta nas configurações.');
      }
    ...
    // Garantir que o caminho está correto para Netlify (.netlify/functions/send-email)
  ...
  async sendTemplate({ tipo, to, cc, vars = {} }) {
    // 1. Obter template
    ...
    // 4. Enviar
    return this.send({ to, cc, subject, body });
  }
};
